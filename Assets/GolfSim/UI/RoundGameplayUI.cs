...
namespace GolfSimZA.UI
{
    public sealed class RoundGameplayUI : MonoBehaviour
    {
        ...
        private GUIStyle brandStyle, holeStyle, courseStyle, labelStyle, valueStyle, mutedStyle;
        private GUIStyle playerNameStyle, activePlayerNameStyle, playerMetaStyle, distanceStyle, shotValueStyle;
        private GUIStyle miniMapStyle, clubStyle, activeClubStyle, actionStyle, panelStyle, panelStrongStyle;
        private GUIStyle topBarStyle, rightCardStyle, metricValueStyle;
        ...
        private GUIStyle MakeLabel(int size, FontStyle fontStyle, Color color, TextAnchor alignment)
        {
            ...
            style.fontSize = size; style.fontStyle = fontStyle; style.alignment = alignment;
            style.normal.textColor = color; style.padding = new RectOffset(0, 0, 0, 0);
            return style;
        }
        ...
        private GUIStyle MakeButton(int size, Color color)
        {
            ...
            style.fontSize = size; style.fontStyle = FontStyle.Bold; style.alignment = TextAnchor.MiddleCenter;
            style.normal.textColor = Color.white; style.normal.background = MakeTexture(color);
            style.hover.textColor = Color.white; style.hover.background = MakeTexture(blueBright);
            style.active.textColor = Color.white; style.active.background = MakeTexture(blue);
        ...
        {
            EnsureStyles();
            DrawTopHUD(); DrawLeftShotOverlay(); DrawRightPlayerOverlay();
            DrawBottomClubBar(); DrawMiniMapOverlay(); DrawShotCompletionAction();
        }

        private void DrawTopHUD()
        {
            float margin = Mathf.Max(18f, Screen.width * 0.018f);
            float barHeight = 52f, width = Screen.width - margin * 2f;
        ...
        {
            float x = 18f, y = 76f, width = Mathf.Clamp(Screen.width * 0.19f, 225f, 285f), height = 252f;
            ...
            bool hasShot = shot.IsValid; float row = y + 60f;
        ...
        private void DrawRightPlayerOverlay()
        {
            ...
            float x = Screen.width - margin - width, y = 76f;
            ...
            for (int i = 0; i < playerNames.Length; i++)
            {
                ...
                if (active) GUI.Box(new Rect(x + 9f, rowY, width - 18f, 67f), GUIContent.none, rightCardStyle);
        ...
        {
            float width = Mathf.Min(440f, Screen.width * 0.40f), x = 18f, y = Screen.height - 92f, h = 70f;
            ...
            for (int i = 0; i < clubs.Length; i++)
            {
                ...
                if (active) GUI.DrawTexture(new Rect(r.x + 3f, r.yMax - 3f, r.width - 6f, 3f), MakeTexture(blueBright));
            }
        }

        private void DrawMiniMapOverlay()
        {
            float margin = Mathf.Max(18f, Screen.width * 0.018f), width = Mathf.Clamp(Screen.width * 0.16f, 190f, 240f);
            float x = Screen.width - margin - width, y = Screen.height - 230f;
        ...
        private void DrawShotCompletionAction()
        {
            ...
            float width = Mathf.Min(420f, Screen.width * 0.38f), x = (Screen.width - width) * 0.5f, y = Screen.height - 74f;
            string text = waitingForNextPlayer ? "NEXT PLAYER  •  " + NextPlayerName() : (holeIndex + 1 < CourseSession.RoundLength ? "NEXT HOLE  •  FINISH HOLE" : "FINISH ROUND");
        ...
        private int GetPlayerRelativeToPar(int playerIndex)
        {
            ...
            int parToCount = 0;
            for (int i = 0; i < holesCompleted; i++) parToCount += parByHole[Mathf.Clamp(i, 0, parByHole.Length - 1)];
            if (playerHoleStrokes[playerIndex] > 0) parToCount += parByHole[Mathf.Clamp(holeIndex, 0, parByHole.Length - 1)];
        ...
        private float DistanceToPin(int playerIndex)
        {
            ...
            Vector3 delta = pin.position - position; delta.y = 0f; return delta.magnitude;
        }

        private void SaveActivePlayerPosition()
        {
            if (ball != null && activePlayerIndex >= 0 && activePlayerIndex < playerPositions.Length) playerPositions[activePlayerIndex] = ball.position;
        }
        ...
        private void AdvancePlayer()
        {
            ...
            shotFinished = false; waitingForNextPlayer = false;
        ...
        private void StartHole()
        {
            ...
            activePlayerIndex = 0; shotFinished = false; waitingForNextPlayer = false;
            ...
            wasInFlight = false; status = "Ready to tee off - " + playerNames[activePlayerIndex];
            for (int i = 0; i < playerPositions.Length; i++) { playerPositions[i] = new Vector3(0f, 0.04f, 0f); playerHoleStrokes[i] = 0; }
            if (ball != null) ball.position = playerPositions[activePlayerIndex];
            if (green != null) { green.position = new Vector3(0f, green.position.y, currentHoleDistance); green.localScale = new Vector3(14f, green.localScale.y, 8f); }
            if (pin != null) pin.position = new Vector3(0f, pin.position.y, currentHoleDistance);
        }

        private void AdvanceHole()
        {
            holesCompleted++; holeIndex++;
            if (holeIndex >= Mathf.Clamp(CourseSession.RoundLength, 1, 18)) { FinishRound(); return; }
            StartHole();
        }
